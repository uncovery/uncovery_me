(function ($) {
	...
	var getByPseudo = function (pseudo, list) {
		var l = list.length;
		for (var i = 0; i < l; i++) {
			if (list[i].pseudo == pseudo) return list[i];
		}
		return false;
	};
	...
	var init = function (res) {
		...
		if (cfg.first_time) {
			...
			for (var i = 0; i < l; i++) {
				...
				// removed
				if (!new_data) {
					if (cfg.users[i].to_remove) continue; // already removed
		...
		for (var i = 0; i < l; i++) {
			var user = users[i];
			var status;
			if (user.is_online) status = '(Online since ' + format(max_time - user.last_time) + ')';
			else status = '(' + format(max_time - user.last_time) + ' ago)';
			
			var line = model.cloneNode(true);
			line.querySelector('.js-pseudo').textContent = user.pseudo;
			if (user.is_online) line.className+= ' online_user';
			else line.className+= ' offline_user';
