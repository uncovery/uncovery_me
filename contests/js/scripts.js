var self_url = "https://uncovery.me/contestsmanager/";
var ss_id = 0;
function umc_postback(url, data, holder, callback) {
    var hidey = $('<div id="hidey"></div>');
    $('#hidey').hide();
    $('#holder').attr('id','holder_sidestep');
    $('body').prepend(hidey);
    $('#hidey').load(
        url,
        data,
        function() {
            ...
            $("#holder_sidestep").html($('#hidey').find('#holder'));
            $('#hidey').remove();
            $("#holder_sidestep").attr('id','holder');
        }
    );
}
...
function umc_create_contest() {
    data = {
        func: "create_contest",
        title: $("#new_contest_title").val(),
        desc: $("#new_contest_desc").val(),
    ...
    umc_postback(self_url, data, "holder");
}

function umc_end_contest(id) {
    data = {
        func: "end_contest",
        id: id
    }
...
function umc_resume_contest(id) {
    data = {
        func: "resume_contest",
        id: id
    }
...
function umc_delete_contest(id) {
    data = {
        func: "delete_contest",
        id: id
    }
...
function umc_delete_entry(cid, id) {
    data = {
        func: "delete_entry",
        contest: cid,
        id: id
...
function umc_enter_contest() {
    screenshots = "";
    $(".screenshot").each(function(index, val) {
        screenshots += val.src + "|";
    });
    data = {
        func: "enter_contest",
        contest: $("#contest_id").val(),
        desc: $("#new_entry_desc").val(),
        screenshots: screenshots,
        title: $("#new_entry_title").val(),
        x: $("#new_entry_x").val(),
        y: $("#new_entry_y").val(),
        z: $("#new_entry_z").val(),
        world: $("#new_entry_world").val(),
    }
    umc_postback(self_url, data, "holder");
}
function umc_list_contests(mode) {
    data = {
        func: "list_contests",
        mode: mode
    }
...
function umc_show_entry(id) {
    ...
    umc_postback(self_url, data, "holder", callback);
}
function umc_vote(id, cat, val) {
    data = {
        func: "vote",
        id: id,
        cat: cat,
...
function umc_change_deadline(button) {
        umc_postback(self_url,
            {func: 'change_deadline',
            deadline: button.prev().val(),
            contest: button.next().val()},
         'holder');
}
function umc_add_screenshot() {
    url = $("#screenshot").val();
    link = $("<a>", {
        href: "javascript:umc_remove_screenshot('" + ss_id + "')",
        id: "ss_" + ss_id++
    });
    img = $("<img>", {
        src: url,
        ...
        load: function() {
            $(this).css("height", 60);
            $(this).attr("class", "screenshot");
            if($.browser.msie) {
                scale = $(this).attr("width")/$(this).attr("height");
                $(this).attr("height", 60);
                $(this).attr("width", 60 * scale);
            }
            $(link).append(this);
...
function umc_remove_screenshot(id) {
    $("#ss_" + id).remove();
}
